import { chooseSetupScanners, formatSetupOutcomes, runSetup } from "./setup.js";
  ...
  return {
    async chooseAction(ctx, overview) {
      const items = [
        ...
        { value: "setup", label: "Setup", description: "Install/configure selected scanners with confirmation" },
...
export function registerControlCommands(pi, dependencies) {
  ...
  const setup = async (args, ctx) => {
    if (!requireTui(ctx, "cloudsec-setup")) return;
    try {
      if (args?.trim()) throw new Error("/cloudsec-setup accepts no arguments; use the scanner selector");
      const selected = await chooseSetupScanners(ctx);
      if (!selected) return;
      const outcomes = await (dependencies.runSetup ?? runSetup)(selected, {
        cwd: ctx.cwd, env: process.env, mode: ctx.mode,
        manualStep: async (recipe) => {
          ctx.ui.notify(`${recipe.reason}\n${recipe.commands.join("\n")}`, "warning");
          const action = await ctx.ui.select("After completing the manual terminal action", ["Re-probe once", "Skip"]);
          return action === "Re-probe once" ? true : action === "Skip" ? "skip" : undefined;
        },
        confirm: (title, body, options) => ctx.ui.confirm(title, body, options),
        execute: (executable, argv, options) => controlUi.run(ctx, "Scanner setup running; Escape cancels", (signal) => dependencies.runSetupProcess(executable, argv, { ...options, signal })),
      });
      ctx.ui.notify(formatSetupOutcomes(outcomes), "info");
  ...
  pi.registerCommand("cloudsec", {
    ...
    handler: async (_args, ctx) => {
      ...
      try {
        ...
        else if (action === "setup") await setup("", ctx);
  ...
  pi.registerCommand("cloudsec-setup", { description: "Guided confirmed local scanner setup (TUI only)", handler: setup });
